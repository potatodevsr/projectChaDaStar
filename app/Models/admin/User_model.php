    ...
    public function getdataTable()
    {
        ...
        return DataTable::of($builder)
            ...
            ->add('status', function ($row) {
                if ($row->status == 0) {
                    $status = '<button type="button" class="btn tool btn-xs btn-round btn-success"><i class="icon wb-check-mini" aria-hidden="true"></i>เปิด</button>';
                } else {
                    $status = '<button type="button" class="btn tool btn-xs btn-round btn-danger"><i class="icon wb-close-mini" aria-hidden="true"></i>ปิด</button>';
                }
                return $status;
            })
